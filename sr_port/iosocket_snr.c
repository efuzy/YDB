...
#	endif
	...
	DBGSOCK2((stdout, "socsnrio: Socket read request - socketptr: 0x"lvaddr"  buffer: 0x"lvaddr"  maxlength: %d  flags: %d  ",
		  socketptr, buffer, maxlength, flags));
	DBGSOCK2((stdout, "time_for_read->tv_sec: %d  usec: %d\n", time_for_read->tv_sec, time_for_read->tv_nsec / NANOSECS_IN_MSEC));
...
#			ifndef GTM_USE_POLL_FOR_SUBSECOND_SELECT
			...
			lcl_time_for_read.tv_usec = (gtm_tv_usec_t)time_for_read->tv_nsec;
			if (pollread)
			{
				readfds = &tcp_fd;
				writefds = NULL;
...
#			else
			...
			poll_nfds = 1;
			poll_timeout = DIVIDE_ROUND_UP(time_for_read->tv_nsec, MICROSECS_IN_MSEC);	/* convert to millisecs */
